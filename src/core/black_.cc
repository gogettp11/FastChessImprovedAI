...
bool in_opp_movelist(int row, int col) {
  ...
  for (int i = 0; i < 2; i++) {
    if (in(white::bishop.movelist[i], pos) ||
        in(white::knight.movelist[i], pos) || in(white::rook.movelist[i], pos))
      return true;
  }
  return false;
}
...
bool castle_criteria_Q() {
  ...
  if (king.moved || rook.moved[0] || !rook.alive[0])
    return false;
  if (white::blocks[0][2] || white::blocks[0][3] || blocks[0][2] ||
      blocks[0][3])
    return false;
...
bool castle_criteria_K() {
  ...
  if (king.moved || rook.moved[1] || !rook.alive[1])
    return false;
  if (white::blocks[0][5] || white::blocks[0][6] || blocks[0][5] ||
      blocks[0][6])
    return false;
  if (white::checker.size() >= 1)
    return false;
...
bool opp_no_moves() {
  ...
  for (int i = 0; i < 2; i++) {
    if (!white::bishop.movelist[i].empty() ||
        !white::knight.movelist[i].empty() || !white::rook.movelist[i].empty())
      return false;
  }
  return true;
}
