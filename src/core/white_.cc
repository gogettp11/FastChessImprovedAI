...
bool in_opp_movelist(int row, int col) {
  ...
  for (int i = 0; i < 8; i++) {
    if (in(black::pawn.hit_movelist[i], pos))
      return true;
  }
  for (int i = 0; i < 2; i++) {
    if (in(black::bishop.movelist[i], pos) ||
        in(black::knight.movelist[i], pos) || in(black::rook.movelist[i], pos))
      return true;
  }
  return false;
}
...
bool castle_criteria_Q() {
  ...
  if (king.moved || rook.moved[0] || !rook.alive[0])
    return false;
  if (black::blocks[7][2] || black::blocks[7][3] || blocks[7][2] ||
      blocks[7][3])
    return false;
...
bool castle_criteria_K() {
  ...
  if (black::blocks[7][6] || black::blocks[7][5] || blocks[7][6] ||
      blocks[7][5]) {
    return false;
  }
  if (black::checker.size() >= 1) {
...
bool opp_no_moves() {
  ...
  for (int i = 0; i < 2; i++) {
    if (!black::bishop.movelist[i].empty() ||
        !black::knight.movelist[i].empty() || !black::rook.movelist[i].empty())
      return false;
  }
  return true;
}
...
void check_avoid_move() {
  ...
  filter2(black::king.movelist, avoid_moves);
}
...
void update_opp_movelists() {
  ...
  for (int i = 0; i < black::num_queens; i++) {
    filter(black::queen.movelist[i], check_movelist);
  }
  for (int i = 0; i < 2; i++) {
    filter(black::bishop.movelist[i], check_movelist);
    filter(black::knight.movelist[i], check_movelist);
    filter(black::rook.movelist[i], check_movelist);
  }
  for (int i = 0; i < 8; i++) {
    filter(black::pawn.movelist[i], check_movelist);
  }
  // the rare case of double check: only the oppposite king can move! (this is
  // because no 1 piece can block both checks)
  if (checker.size() > 1) {
    for (int i = 0; i < 8; i++) {
...
void play() {
  if (queue.row.size() >= 2 && blocks[queue.row[0]][queue.col[0]] == 1 &&
    ...
    log << "-----------------------NON-AI MOVE-----------------------"
        << std::endl;
    log << "(" << piece << " " << queue.row[1] << " " << queue.col[1] << ")"
        << std::endl;
    log << "---------------------------------------------------------"
        << std::endl;
    log << "---------------------------------------------------------"
...
void move_piece(std::string piece, int row, int col) {
  ...
  for (int i = 0; i < num_queens; i++) {
    if (piece == "Q" + std::to_string(i) && in(queen.movelist[i], pos)) {
      ...
      moved = true;
    } else if (piece == "Q" + std::to_string(i) &&
               !in(queen.movelist[i], pos)) {
      sound.error();
      return;
    }
  }
  ...
  for (int i = 0; i < 2; i++) {
    if (piece == "B" + std::to_string(i) && in(bishop.movelist[i], pos)) {
      ...
      moved = true;
    } else if (piece == "B" + std::to_string(i) &&
               !in(bishop.movelist[i], pos)) {
      sound.error();
      return;
    }
...
void valid_move(bool is_undo, bool killed, std::string piece, int row,
                int col) {
...
#ifdef SCREENSHOTS_ON
  if (!board.isFrozen && screenshots_on)
    screenshot = true;
#endif
#ifndef IS_TESTING
  turn = false;
