...
#ifdef VERBOSE2
  std::cout << "Undoing " << undo.piece[last] << " " << undo.moved_from[last][0]
  ...
  if (undo.color[last] == "W") {
    ...
    for (int i = 0; i < white::num_queens; i++) {
      if (undo.piece[last] == "Q" + std::to_string(i))
        white::queen.move(i, undo.moved_from[last][0],
                          undo.moved_from[last][1]);
    }
    for (int i = 0; i < 8; i++) {
      if (undo.piece[last] == "P" + std::to_string(i)) {
        if (undo.moved_from[last][0] == 1)
          white::handle_undo_promotion(i, undo.moved_from[last][0],
                                       undo.moved_from[last][1]);
    ...
    for (int i = 0; i < 2; i++) {
      if (undo.piece[last] == "B" + std::to_string(i))
        white::bishop.move(i, undo.moved_from[last][0],
                           undo.moved_from[last][1]);
      else if (undo.piece[last] == "N" + std::to_string(i))
        white::knight.move(i, undo.moved_from[last][0],
                           undo.moved_from[last][1]);
    ...
    if (undo.killed[last] && undo.killed_color[last] == "B")
      black::revive(undo.killed_piece[last], undo.killed_pos[last][0],
                    undo.killed_pos[last][1]);
    if (!board.isFrozen)
      white::reset_enpassant();
    white::valid_move(true, undo.killed[last], undo.piece[last],
                      undo.moved_from[last][0], undo.moved_from[last][1]);
    ...
    for (int i = 0; i < black::num_queens; i++) {
      if (undo.piece[last] == "Q" + std::to_string(i))
        black::queen.move(i, undo.moved_from[last][0],
                          undo.moved_from[last][1]);
    }
    for (int i = 0; i < 8; i++) {
      if (undo.piece[last] == "P" + std::to_string(i)) {
        if (undo.moved_from[last][0] == 6)
          black::handle_undo_promotion(i, undo.moved_from[last][0],
                                       undo.moved_from[last][1]);
    ...
    for (int i = 0; i < 2; i++) {
      if (undo.piece[last] == "B" + std::to_string(i))
        black::bishop.move(i, undo.moved_from[last][0],
                           undo.moved_from[last][1]);
      else if (undo.piece[last] == "N" + std::to_string(i))
        black::knight.move(i, undo.moved_from[last][0],
                           undo.moved_from[last][1]);
      else if (undo.piece[last] == "R" + std::to_string(i))
        black::rook.move(i, undo.moved_from[last][0], undo.moved_from[last][1]);
    }
    if (undo.killed[last] && undo.killed_color[last] == "W")
      white::revive(undo.killed_piece[last], undo.killed_pos[last][0],
                    undo.killed_pos[last][1]);
    if (!board.isFrozen)
      black::reset_enpassant();
    black::valid_move(true, undo.killed[last], undo.piece[last],
                      undo.moved_from[last][0], undo.moved_from[last][1]);
  }
  checkmate = false;
  stalemate = false;
...
void Game_Board::draw_board() {
  ...
  for (int k = 0; k < 8; k++) {
    ...
    for (int i = 0; i < 8; i++) {
      if (outer) {
        (i % 2 == 0) ? drawRect(brown, x, y, UNIT, UNIT)
                     : drawRect(white, x, y, UNIT, UNIT);
        x += UNIT;
      } else {
        (i % 2 == 0) ? drawRect(white, x, y, UNIT, UNIT)
...
void Game_Board::Circle(int row, int col) {
  ...
  circle.setPosition(col * UNIT + 2.3 * board.pieces_paddingx,
                     row * UNIT + 3.8 * board.pieces_paddingy);
