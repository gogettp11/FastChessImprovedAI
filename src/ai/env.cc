...
void init(const std::string &env) {
  if (env == "")
    ...
    for (int i = 0; i < 2; i++) {
      black::kill(true, "B" + str(i), black::bishop.row[i],
                  black::bishop.col[i]);
      black::kill(true, "R" + str(i), black::rook.row[i], black::rook.col[i]);
      black::kill(true, "N" + str(i), black::knight.row[i],
                  black::knight.col[i]);
    }
    white::kill(true, "Q0", 7, 3);
    white::kill(true, "K", 7, 4);
    for (int i = 0; i < 2; i++) {
      white::kill(true, "B" + str(i), white::bishop.row[i],
                  white::bishop.col[i]);
      white::kill(true, "R" + str(i), white::rook.row[i], white::rook.col[i]);
      white::kill(true, "N" + str(i), white::knight.row[i],
                  white::knight.col[i]);
    }
