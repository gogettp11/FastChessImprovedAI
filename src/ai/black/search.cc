...
static const double minimax(const int &n, double alpha, double beta,
                            std::string player) {
  std::map<std::string, std::vector<std::vector<int>>> black_moves =
      black::get_moves();
  std::map<std::string, std::vector<std::vector<int>>> white_moves =
      white::get_moves();
  ...
  if (player == "B") {
    ...
    for (const auto &[k, v] : btemp) {
      ...
      for (int i = 0; i < value.size(); i++) {
        if (piece == "CK" || piece == "CQ")
          continue;
        black::move_piece(piece, value[i][0], value[i][1]);
#ifdef VERBOSE2
        print("INNER BLACK");
        std::cout << piece << " " << value[i][0] << " " << value[i][1]
                  << std::endl;
#endif
#ifdef DEBUGAI
...
void gen_move() {
...
#endif
  std::map<std::string, std::string> best_move = {
      {"score", "-9999"}, {"piece", ""}, {"pos", ""}};
  ...
  const std::vector<bool> W_rook_moved = {white::rook.moved[0],
                                          white::rook.moved[1]};
  const bool B_king_moved = black::king.moved;
  const std::vector<bool> B_rook_moved = {black::rook.moved[0],
                                          black::rook.moved[1]};
  ...
  if (best_move["piece"] == "CK")
    ...
    black::move_piece(best_move["piece"], best_move["pos"][0] - '0',
                      best_move["pos"][1] - '0');
#ifdef VERBOSE
  std::cout << "Best move is " << best_move["piece"] << " to "
            << "(" << best_move["pos"][0] << ", " << best_move["pos"][1] << ")"
            << std::endl;
#endif
#ifndef IS_TESTING
  black::turn = false;
